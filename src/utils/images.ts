...
export const parseExcludeList = (raw: string): number[] => {
  if (!raw.trim()) return []
...
const probeImage = (url: string, signal?: AbortSignal): Promise<ProbeResult> =>
  new Promise((resolve) => {
    ...
    img.onload = () =>
      finish({ ok: true, width: img.naturalWidth || 1, height: img.naturalHeight || 1 })
